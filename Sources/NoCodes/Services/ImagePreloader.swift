...
final class ImagePreloader: ImagePreloaderInterface {
  ...
  /// Maximum time a single image download may take before it is abandoned.
  ...
  /// A slow image must not hold up the screen, so the download carries its own
  /// deadline: exceeding it leaves the original URL in the HTML.
  private func downloadAndConvert(url urlString: String) async -> (String, String?) {
    ...
    guard let (data, response) = await download(request) else {
      return (urlString, nil)
    }
  ...
  /// Runs the download against a wall-clock deadline: the request timeout alone
  /// only bounds the gaps between packets, a trickling response could still
  /// stall the screen indefinitely.
  private func download(_ request: URLRequest) async -> (Data, URLResponse)? {
    let session: URLSession = urlSession
    let deadline: UInt64 = UInt64(timeout * 1_000_000_000)

    return await withTaskGroup(of: (Data, URLResponse)?.self) { group in
      group.addTask {
      ...
      let firstFinished: (Data, URLResponse)? = await group.next() ?? nil
      group.cancelAll()
